...
final class SiteHealthExtensions implements ModuleInterface {
    ...
    public function checks(): array {
        return [
            $this->check('https', $this->https()),
            $this->check('debug', $this->debug()),
            $this->check('search_visibility', $this->visibility()),
            $this->check('cron', $this->cron()),
        ];
    }
    ...
    private function check(string $id, array $result): array {
        return [
            'id' => $id,
            'status' => (string) ($result['status'] ?? 'recommended'),
            'message' => wp_strip_all_tags((string) ($result['label'] ?? '')),
        ];
    }
