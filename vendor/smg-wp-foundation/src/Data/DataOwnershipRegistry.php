...
final class DataOwnershipRegistry {
    ...
    public function register(string $module,string $type,string $key,string $policy='preserve'):self{
        $module=self::key($module);$type=self::key($type);$key=trim($key);$policy=self::key($policy);
    ...
    public function module(string $module):array{return $this->owners[self::key($module)]??[];}
